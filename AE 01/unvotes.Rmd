...
D because it is initially stable then increases and decreases unexpectedly, forming a straight pattern, then rapidly making a bump.

**Task 2.2:** Which country appears most consistent in its voting patterns across all issues? Which appears least consistent?

**Your answer:**

I think turkey is the most consistent because the shape it forms, similar to an M, is consistent in all of them. I think the united states is the least consistent. i believe so because i tried visually analyzing the distance between the lowest and highest points on the graphs and felt like the united states generally had a larger distance.
...
unvotes %>%
  filter(country %in% c("China", "Russia", "India")) %>%  # Change these!
  mutate(year = year(date)) %>%
  group_by(country, year, issue) %>%
  summarize(percent_yes = mean(vote == "yes")) %>%
  ggplot(mapping = aes(x = year, y = percent_yes, color = country)) +
...
**Which countries did you choose?** China, Russia, and India. 

**Task 3.2:** Describe one interesting pattern you notice in your new plot.

**Your answer:**

one observation is that china has the highest average measure of positive human rights votes.
...
unvotes %>%
  filter(country %in% c("China", "Russia", "India", "United States", "United Kingdom")) %>%  # Choose 5 countries
  ...
  scale_y_continuous(labels = percent) +
  labs(
    title = "Human Rights Voting Patterns in the UN General Assembly",  # Update title
    subtitle = "1946 to 2019",
    y = "% Yes",
    x = "Year",
    color = "Country"
...
One thing i noticed is that the trends behind each countries' lines always change for each country due to different issues. China generally seems to have higher percentages of yes votes compared to Russia and India due to different issues, for example.
...
**Which issue has the most "yes" votes on average?** Palestinian conflict_____________________

**Which has the least?** Humanrights
---
